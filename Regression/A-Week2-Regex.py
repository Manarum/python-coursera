...
#Data Files We provide two files for this assignment.
...
#Sample data: http://py4e-data.dr-chuck.net/regex_sum_152029.txt (There are 102 values and the sum ends with 392)
#Actual data: http://py4e-data.dr-chuck.net/regex_sum_910137.txt (There are 78 values and the sum ends with 674)
...
#Data FormatThe file contains much of the text from the introduction of the textbook except that random numbers are inserted throughout the text.
...
#Handling The DataThe basic outline of this problem is to read the file, look for integers using the re.findall(),
...
import re
fhandle=open('regex_sum_910137.txt')
ls=list()
for line in fhandle:
    line=line.rstrip()
